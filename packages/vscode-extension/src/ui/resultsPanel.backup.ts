...
<head>
  ...
  <style>
    ...
    thead th{text-align:left;font-weight:600;color:var(--muted);border-bottom:1px solid var(--border);padding:6px 8px;white-space:nowrap;position:sticky;top:0;background:var(--panel);z-index:1;}
    ...
    details.inline-block{display:inline-block;}
    ...
    .btn{display:inline-flex;align-items:center;gap:6px;font-size:12px;border:1px solid var(--border);background:transparent;padding:4px 8px;border-radius:6px;cursor:pointer;}
    .btn:hover{border-color:var(--accent);}
    .toolbar{display:flex;gap:8px;margin-bottom:8px;}
    /* naive diff tinting */
    .diff pre ins{background:rgba(59,127,59,0.18); text-decoration:none;}
    .diff pre del{background:rgba(181,84,84,0.22); text-decoration:none;}
  </style>
</head>
<body>
  <div class="wrap">
    ...
    <div class="card">
      <div class="toolbar">
        <button class="btn" id="copy-raw">copy</button>
      </div>
      <pre id="raw-json"></pre>
    </div>
  </div>
    ...
    function renderSmartTable(selector, src){
      ...
      const tbody = '<tbody>' + rows.map(r=>{
        return '<tr>' + cols.map(c=>{
          ...
          // Patch: show "view diff" and "view JSON" when available
          if (c==='patch' && r.patch && typeof r.patch==='object'){
            const hasDiff = typeof r.patch.diff === 'string' && r.patch.diff.trim().length > 0;
            let pretty=''; try{ pretty=JSON.stringify(r.patch,null,2);}catch{ pretty=String(r.patch); }

            const diffHtml = hasDiff
              ? [
                  '<details class="inline-block diff" style="margin-right:8px;">',
                  '  <summary>view diff</summary>',
                  '  <pre>',
                  escapeHtml(r.patch.diff),
                  '  </pre>',
                  '</details>'
                ].join('')
              : '';
            ...
            return '<td>' + diffHtml + jsonHtml + '</td>';
          }
